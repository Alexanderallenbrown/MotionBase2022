...
const int jogmodePin = 12;
const int openloopPin = 13;
...
static int globzeros[6] = {500,850,450,500,500,500};
...
void loop()
{
  ...
  jogmode=digitalRead(jogmodePin);
  openloop = digitalRead(openloopPin);
  ...
  if (jogmode==false){
   // static long unCount;
    ...
    if (millis()>5000)
    {
      ref_command_float = (val - zero[motornum-1])/servo_mult; //this should give the reference position, converted from microseconds to radians
    }
  }
  ...
  if (ref_command_float>1.0){
    ref_command_float = 1.0;
  }
  if (ref_command_float<-1.0){
    ref_command_float = -1.0;
  }
  ...
  float float_U = 0;
  if(openloop==false){
  float_U = kp*float_error;
  }
  else{
  float_U = (analogRead(analogInPin)-512)*255.0/512.0; 
  }
...
//raw reference position value 0-1023 (open or closed loop)
  Serial.print(val);
  Serial.print("\t");
  //raw potentiometer position
  Serial.print(rawpos);
  Serial.print("\t");
  //position estimate in radians based on potentiometer
    Serial.print(posfloat);
  Serial.print("\t");
  //position estimate based on encoder (should match above)
      Serial.print(unCount_float);
  Serial.print("\t");
  //magnitude of motor command
  Serial.print(mag);
//  Serial.print("\t");
//  Serial.print(val2);
...
//Serial.print(ref_command_float*180/PI);
//Serial.print("\t");
//Serial.print(posfloat*180/PI);
//Serial.print("\t");
//Serial.println(unCount_float*180/PI);
